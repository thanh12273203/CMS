...
void DetFrameProducer::fillTRKlayersAtECALstitchedTID ( const edm::Event& iEvent, const edm::EventSetup& iSetup, unsigned int proj ) {
    ...
    for ( int iL(0); iL < nTID; iL++ ) {
      ...
      if (proj==0){
        ...
        for ( int iz(0); iz < nEE; iz++ ) {
          ...
          sprintf(hname, "evt_TID_layer%d_EE%s",layer_,zside);
...
//    std::cout << "Strip Id = " << id << " - subId ( 3->TIB | 4->TID | 5->TOB | 6->TEC ) = " << subid << " - Layer = " << layer <<  std::endl;
  ...
  ffillTRKLayerAtECAL_with_EEproj( hEvt_EE_TID, vTID_ECAL_, hTID_ECAL, nTID, proj);
